...
#建回测时间结束
    ...
    for code in stockList:#循环股票
        ...
        for backTraceDate in date_seq:#循环日期
            ...
            if ((isShow==True) | (not len(df))):
                continue
            dfTemp=df.tail(1)#取最后一行
            # dfTemp['code']=[code]
            if 'code' not in list(dfTemp):
                dfTemp.insert(0, 'code', [code])  
            dfTemp['buy_sellNum']=[0]
            dfTemp['holdNum']=[0]   
            # dfTemp['profit']=[0]      
            ...
            buyBIAS=(dfTemp['buyBIAS'].isnull().any()==False)
            sellBoll=(dfTemp['sellBoll'].isnull().any()==False)
            sellDownMA20=(dfTemp['sellDownMA20'].isnull().any()==False)
            sellBIAS=(dfTemp['sellBIAS'].isnull().any()==False)
            if (buyUpMA20 | buyValleyMA20 | buyBIAS):
                money=0
                if buyValleyMA20:
                    money=40000
                else:
